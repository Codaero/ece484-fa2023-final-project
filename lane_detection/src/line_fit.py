...
def line_fit(binary_warped):
	...
	rect_color = (0, 255, 0) # rectangle is green color
	thickness = 3 # 3 pixel thick
	# Step through the windows one by one
	for window in range(nwindows):
		print(f"Left: {leftx_current} Right: {rightx_current}" )
		# Identify window boundaries in x and y (and right and left)
		## TODO
		lbox_left = max(0, leftx_current - margin)
		lbox_right = leftx_current + margin

		rbox_left = rightx_current - margin
		rbox_right = min(rightx_current + margin, binary_warped.shape[1])
		...
		left_start = np.array([lbox_left, window_top])
		left_end = np.array([lbox_right, window_bottom])
		
		right_start = np.array([rbox_left, window_top])
		right_end = np.array([rbox_right, window_bottom])

		out_img = cv2.rectangle(out_img, left_start, left_end, rect_color, thickness)
		out_img = cv2.rectangle(out_img, right_start, right_end, rect_color, thickness)

		####
		# Identify the nonzero pixels in x and y within the window
		## TODO
	
		# Boolean numpy array for nonzero pixels in our boxes
		filtered_left = (nonzerox < lbox_right) & (nonzerox > lbox_left) & (nonzeroy < window_bottom) & (nonzeroy > window_top)
		filtered_right = (nonzerox < rbox_right) & (nonzerox > rbox_left) & (nonzeroy < window_bottom) & (nonzeroy > window_top)

		# Indices in the nonzerox and nonzeroy arrays
		nonzero_left = np.nonzero(filtered_left)[0]
		nonzero_right = np.nonzero(filtered_right)[0]

		# nonzero_filtered_left = np.array([[x, y] for [x, y] in nonzero_packed if x < lbox_right and x > lbox_left and y < window_bottom and y > window_top])
		# nonzero_filtered_right = np.array([[x, y] for [x, y] in nonzero_packed if x < rbox_right and x > rbox_left and y < window_bottom and y > window_top])
	...
	## TODO
		...
		# Fit a least-squares 2nd order polynomial to both boxes
